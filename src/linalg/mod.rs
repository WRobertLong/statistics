...
/// 
/// # Arguments
/// * `x` - The design matrix (n x p). MUST include a column of 1s if you want an intercept.
/// * `y` - The target vector (n x 1).
/// 
/// # Formula
/// beta = (X^T * X)^-1 * X^T * y
pub fn ols(x: &Matrix, y: &Matrix) -> Result<Matrix, StatsError> {
    // 1. Calculate X^T
    let xt = x.transpose();

    // 2. Calculate Gram Matrix: X^T * X
    // If this is singular, we have perfect multicollinearity.
